...
{
    this->headSector = sector;
    hdr = new FileHeader;
    hdr->FetchFrom(sector);
    //hdr->Print();
   // hdr->printHeader();
    seekPosition = 0;
    this->preLength = this->hdr->getNumBytes();
}
...
{
    // 判断长度是否修改
    if (this->preLength != this->hdr->getNumBytes()) {
        if (this->preLength) {
            time_t timep;
            time(&timep);
            this->hdr->setTime((int) timep);
        }
        WriteBack();
    }
    delete hdr;
...
{ 
    ...
    int fileLength = hdr->FileLength();
    //printf("fileLength:%d position :%d  numBytes:%d",fileLength,position,numBytes);
    int i, firstSector, lastSector, numSectors;
    ...
    if ((position + numBytes) > fileLength) {
        // printf("\njinru\n");
		if (!hdr->extend(position+numBytes)) {
            return 0;
        }else{
            WriteBack();
          //  printf("\ntuzhan\n");
