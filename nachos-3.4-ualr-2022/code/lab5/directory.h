...
class DirectoryEntry {
  ...
  //1 B
    bool inUse;
  //4 B  				// Is this directory entry in use?
    int sector;				// Location on disk to find the 				//   FileHeader for this file 
  //9 bit is authority  32-9=23 bits are useId
    int authority;
  //10 B
...
class Directory {
  public:
    ...
    void PrintTotal();
    void setAuthority(char *name, int Kinds);
    bool isHasAuthority(AccountEntity *,OpenFile*,char *name,AuthorityKind kind);
  private:
