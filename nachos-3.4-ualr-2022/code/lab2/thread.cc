...
Thread::Thread(char* threadName, int threadpriority){
    ...
    stackTop = NULL;
    status = JUST_CREATED;
    if(threadpriority > MAX_PRIORITY){ //0-99静态优先级
    	priority = MAX_PRIORITY;
    }
    else if(threadpriority < MIN_PRIORITY){
    	priority = MIN_PRIORITY;
    }
    else priority = threadpriority;
...
//thread中涉及到多个线程的操作主要有fork和yield。
//----------------------------------------------------------------------
// Thread::~Thread
...
//首先考虑fork函数。
//因其只调用了readytorun函数且不涉及其他多线程调度操作，所以无需修改。
void 
...
//然后考虑yield函数。
//线程在yield时先把自己放入就绪队列还是先寻找下一个线程对于nachos原来的fcfs调度来说并没有影响，因为获得调度的下一线程肯定不是当前线程。
//但是优先级调度中获得调度的下一线程有可能是当前线程，所以规定线程在yield时必须先把自己放入就绪队列之后再寻找下一个线程。
void
Thread::Yield ()
{
    Thread *nextThread;
...
// =======================================2.实现带有优先级的线程操作============================================
    //调整进程调度顺序
    scheduler->ReadyToRun(this);//先把当前进程放入就绪队列
    nextThread = scheduler->FindNextToRun();//后把就绪队列首位赋给nextThread
// =======================================2.实现带有优先级的线程操作============================================
